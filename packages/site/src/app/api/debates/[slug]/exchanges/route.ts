import { NextRequest, NextResponse } from "next/server";
import { db } from "@/db";
import { comments, users, debates } from "@/db/schema";
import { eq } from "drizzle-orm";
import { redis } from "@/lib/redis";
import { findBestExchanges } from "@/lib/exchange-pairing";
import type { ExchangePair } from "@/types/exchanges";

export async function GET(
  request: NextRequest,
  { params }: { params: Promise<{ slug: string }> }
) {
  try {
    const { slug } = await params;
    const { searchParams } = new URL(request.url);
    const sort = searchParams.get("sort") ?? "score";

    // Find debate by slug
    const [debate] = await db
      .select({ id: debates.id })
      .from(debates)
      .where(eq(debates.slug, slug))
      .limit(1);

    if (!debate) {
      return NextResponse.json({ error: "Debate not found" }, { status: 404 });
    }

    // Check Redis cache
    const cacheKey = `debate:${debate.id}:exchanges`;
    const cached = await redis.get(cacheKey);
    let exchanges: ExchangePair[];

    if (cached) {
      exchanges = JSON.parse(cached) as ExchangePair[];
    } else {
      // Fetch all comments for this debate with author info
      const rows = await db
        .select({
          id: comments.id,
          debateId: comments.debateId,
          authorId: comments.authorId,
          parentId: comments.parentId,
          rootId: comments.rootId,
          depth: comments.depth,
          content: comments.content,
          stanceSide: comments.stanceSide,
          ancestorPath: comments.ancestorPath,
          isQuarantined: comments.isQuarantined,
          quarantineReason: comments.quarantineReason,
          status: comments.status,
          score: comments.score,
          createdAt: comments.createdAt,
          editedAt: comments.editedAt,
          authorDisplayName: users.displayName,
          authorUsername: users.username,
          authorAvatarUrl: users.avatarUrl,
        })
        .from(comments)
        .innerJoin(users, eq(comments.authorId, users.id))
        .where(eq(comments.debateId, debate.id))
        .orderBy(comments.createdAt);

      const commentsWithAuthor = rows.map((row) => ({
        id: row.id,
        debateId: row.debateId,
        authorId: row.authorId,
        parentId: row.parentId,
        rootId: row.rootId,
        depth: row.depth,
        content: row.content,
        stanceSide: row.stanceSide,
        ancestorPath: row.ancestorPath,
        isQuarantined: row.isQuarantined,
        quarantineReason: row.quarantineReason,
        status: row.status,
        score: row.score,
        createdAt: row.createdAt.toISOString(),
        editedAt: row.editedAt?.toISOString() ?? null,
        author: {
          id: row.authorId,
          displayName: row.authorDisplayName,
          username: row.authorUsername,
          avatarUrl: row.authorAvatarUrl,
        },
      }));

      exchanges = findBestExchanges(commentsWithAuthor);

      // Cache for 60 seconds
      await redis.set(cacheKey, JSON.stringify(exchanges), "EX", 60);
    }

    // Apply sorting
    if (sort === "recent") {
      exchanges.sort((a, b) => {
        const aLatest = a.comments[a.comments.length - 1].createdAt;
        const bLatest = b.comments[b.comments.length - 1].createdAt;
        return bLatest.localeCompare(aLatest);
      });
    }
    // "shifts" falls back to score (default order from algorithm)
    // "score" is already the default sort from findBestExchanges

    return NextResponse.json({ exchanges });
  } catch (error) {
    console.error("GET /api/debates/[slug]/exchanges error:", error);
    return NextResponse.json(
      { error: "Internal server error" },
      { status: 500 }
    );
  }
}
